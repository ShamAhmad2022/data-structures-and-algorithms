...
function leftJoin(hm1, hm2) {
  ...
  hm1.keys().forEach((key) => {
    const synonym = hm1.get(key);
    const antonym = hm2.has(key) ? hm2.get(key) : null;

    leftJoin.push([key, synonym, antonym]);
  });
...
hashTable2.map.map((data, index) => {
  console.log(index, data && data.print());
});


console.log(hashTable2.has('Niko'));

console.log(hashTable2.get('Niko'));
